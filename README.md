...
打开一个网页，侧边栏里 Sabrina 已经知道你在看什么。不需要复制，不需要描述，不需要粘贴链接。

**典型场景：**

- 📖 **研究对比** — 同时打开 5 个竞品页，跨标签引用，一次生成对比结论
- 📝 **内容提炼** — 看长文时直接问"总结重点"，上下文自动注入，不用全选复制
- ⚡ **Skill 直达** — 当前页面作为输入，直接触发 OpenClaw skill，写周报、翻译、提 issue 一步到位
- 🗂️ **GenTab** — 选中多个参考页，一键生成结构化结果页（对比表 / 列表 / 时间线）
- 🧵 **线程记忆** — 关掉再打开，上下文还在；对话历史按页面和站点自动归档
